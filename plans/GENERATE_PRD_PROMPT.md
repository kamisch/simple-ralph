...
*   `context`: (Optional but recommended) Detailed technical context, file paths to modify, or specific constraints like what task this is dependent on.
...
3.  **Generate JSON**: Output purely the JSON array. Do not wrap it in markdown code blocks if possible, or strictly use `msg` blocks if required by the interface.
